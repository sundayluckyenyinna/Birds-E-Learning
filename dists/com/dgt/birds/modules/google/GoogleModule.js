"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
const common_1 = require("@nestjs/common");
const UserSQLRepository_1 = require("../user/repository/UserSQLRepository");
const MessageSource_1 = require("../../config/MessageSource");
const GoogleDriveService_1 = require("./service/GoogleDriveService");
const UserProfilePicSQLRepository_1 = require("../user/repository/UserProfilePicSQLRepository");
const typeorm_1 = require("@nestjs/typeorm");
const UserProfilePicture_1 = require("../user/model/UserProfilePicture");
let GoogleModule = class GoogleModule {
};
GoogleModule = __decorate([
    (0, common_1.Module)({
        imports: [typeorm_1.TypeOrmModule.forFeature([UserProfilePicture_1.default])],
        providers: [
            UserSQLRepository_1.default, MessageSource_1.default, GoogleDriveService_1.default,
            UserProfilePicSQLRepository_1.default
        ]
    })
], GoogleModule);
exports.default = GoogleModule;
//# sourceMappingURL=GoogleModule.js.map